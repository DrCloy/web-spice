...
feature-architect가 태스크를 분석하고 Codex / Codex 트랙으로 분리.
Codex 트랙: engine-coder + ui-coder (병렬) → qa-validator
...
Agent(
  model: "opus",
  prompt: ".Codex/agents/feature-architect.md를 읽고 다음 요청을 분석하여
           _workspace/plan.md를 작성하라. Codex / Codex 트랙을 분리할 것:
           {사용자 요청}"
)
```
...
## Phase 2A: Codex 트랙 (engine-coder + ui-coder)

`_workspace/plan.md`의 Codex 트랙을 기반으로 실행 경로 결정:
...
- 에이전트 정의 파일 경로 (`.Codex/agents/{name}.md` 읽도록 지시)
- 관련 스킬 파일 (spice-engine 또는 spice-ui)
- `_workspace/plan.md`의 Codex 트랙 내용
...
**Codex 트랙과 Codex 트랙은 독립적이므로 동시에 진행 가능.**
단, 공유 타입 변경이 있으면 Codex가 먼저 처리 후 Codex 트랙 시작.

## Phase 3: QA 검증 (qa-validator, 점진적)

Codex 트랙 완료 시마다 qa-validator 실행:

```
Agent(
  model: "opus",
  subagent_type: "general-purpose",
  prompt: ".Codex/agents/qa-validator.md를 읽고 현재 구현을 검증하라.
...
- Codex 트랙: 구현 파일 목록, 테스트 결과, CI 통과 여부
...
→ architect: 캐패시터/인덕터 → Codex, AC 분석 → Codex
→ 공유 타입 먼저 처리 → 두 트랙 병렬 진행
→ Codex 완료 후 qa-validator → Codex는 자체 PR 생성
