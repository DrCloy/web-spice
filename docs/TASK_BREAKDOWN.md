...
| #2  | TASK | ✅ 테스트 유틸리티 및 헬퍼 함수 구현    | TDD를 위한 테스트 헬퍼, 모크 데이터 생성기                 | HIGH     | #1     |
| #3  | TASK | ✅ 프로젝트 폴더 구조 및 개발 환경 검증 | 실제 개발 시작 전 환경 최종 검증                           | MED      | -      |

### Component Models (TDD 기반)

| ID  | 타입 | 제목                                        | 설명                                             | 우선순위 | 의존성     |
| --- | ---- | ------------------------------------------- | ------------------------------------------------ | -------- | ---------- |
| #4  | FEAT | ✅ 저항(Resistor) 컴포넌트 모델 구현        | 옴의 법칙 기반 저항 모델, 입력 검증, 단위 테스트 | HIGH     | #1, #2     |
| #5  | FEAT | ✅ 전압원(VoltageSource) 컴포넌트 모델 구현 | DC 전압원 모델, 파라미터 검증                    | HIGH     | #1, #2     |
| #6  | FEAT | ✅ 전류원(CurrentSource) 컴포넌트 모델 구현 | DC 전류원 모델, 극성 관리                        | HIGH     | #1, #2     |
| #7  | FEAT | ✅ 기본 회로(Circuit) 클래스 구현           | 컴포넌트 관리, 노드 연결, 회로 검증              | HIGH     | #4, #5, #6 |

### Numerical Engine

| ID  | 타입 | 제목                                        | 설명                                       | 우선순위 | 의존성  |
| --- | ---- | ------------------------------------------- | ------------------------------------------ | -------- | ------- |
| #8  | FEAT | ✅ 행렬 연산 유틸리티 구현 (Matrix, Vector) | 기본 행렬 연산, 희소행렬 지원, 수치 안정성 | HIGH     | #1, #2  |
| #9  | FEAT | ✅ LU 분해 알고리즘 구현                    | 피벗팅 포함 LU 분해, 특이행렬 처리         | HIGH     | #8      |
| #10 | FEAT | ✅ 노달 분석 DC 해석기 구현                 | Modified Nodal Analysis 기반 DC 해법       | HIGH     | #7, #9  |
| #11 | FEAT | ✅ Newton-Raphson 수렴 알고리즘 구현        | 비선형 해석을 위한 반복 해법, 수렴 조건    | HIGH     | #9, #10 |

### I/O & Integration

| ID  | 타입 | 제목                           | 설명                                                                  | 우선순위 | 의존성   |
| --- | ---- | ------------------------------ | --------------------------------------------------------------------- | -------- | -------- |
| #12 | FEAT | ✅ JSON 회로 파서 구현         | JSON 형식 회로 정의 파싱, 오류 처리                                   | HIGH     | #7       |
| #13 | FEAT | ✅ 시뮬레이션 결과 포매터 구현 | 결과 데이터 구조화, 포맷팅, 내보내기 (DC Sweep 분석 포함, dcSweep.ts) | HIGH     | #10, #11 |
| #14 | TEST | ✅ 전체 DC 분석 통합 테스트    | End-to-end DC 분석 워크플로 검증                                      | HIGH     | #12, #13 |
| #15 | FEAT | ✅ 콘솔 출력 시스템 구현       | 개발자 친화적 결과 표시, 디버깅 정보 (CLI, examples/)                 | MED      | #13      |
...
| #36 | FEAT | 파일 저장/불러오기 시스템    | .wspice 형식, 브라우저 파일 API, 표준 SPICE 넷리스트(.cir/.spice) import/export | HIGH     | #13      |
...
Last Updated: 2026-04-06 (Task #18 컴포넌트 팔레트 UI 구현 완료 — ThemeProvider, IEEE 심볼, CanvasColors 포함)
